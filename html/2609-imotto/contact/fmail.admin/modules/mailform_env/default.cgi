...
if($g_recaptcha_flag){
	$g_recaptcha_flag_checked = " checked";
}

$resend_mail_address_toiawase = '';
$resend_mail_address_sashidashi = '';
if ($resend_mail_address eq 'toiawase') {
	$resend_mail_address_toiawase = 'checked';
}
if ($resend_mail_address eq 'sashidashi') {
	$resend_mail_address_sashidashi = 'checked';
}
...
for($cnt=0;$cnt<@elements;$cnt++){
	...
	if($element_type ne "spacer"){
		#既に登録されている値から、selectedを指定
		$checkflag_resend_mail_address = ${resend_mail_address};
		$checkflag_resend_mail_address =~ s/&lt;//g;
		$checkflag_resend_mail_address =~ s/&gt;//g;
		if($checkflag_resend_mail_address eq ${elements_id}){
			$add_elements_value_list_resend_mail_address .= "<option value=\"<${elements_id}>\" selected=\"selected\">${name}　<${elements_id}></option>";
			#エンドユーザ情報が反映されていたので、フラグに1を立てる
			$flag_resend_mail = 1;
		}else{
			$add_elements_value_list_resend_mail_address .= "<option value=\"<${elements_id}>\">${name}　<${elements_id}></option>";
		}
	}
}
$add_elements_value_list_resend_mail_address = "<option value=\"\" class=\"caution\">設定なし </option>" . $add_elements_value_list_resend_mail_address;
...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" onSubmit="return checkUserEdit(this,'${submit}');">
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			<th>管理者宛メールの<br />返信先</th>
			<td>
			    <div>
                    <select name="resend_mail_address" id="resend_mail_address">
                        ${add_elements_value_list_resend_mail_address}
                    </select>
			    </div>
				<span class="caution">返信先のメールアドレスとして利用します。</span><br />
			</td>
		</tr>
		<tr>
...
-->
	...
	<p class="highleveldata">Google reCAPTCHA設定</p>
	<table cellpadding="0" cellspacing="0" class="sheet highleveldata">
		<tr>
			<th>reCAPTCHAを利用する</th>
			<td>
				<input type="checkbox" name="g_recaptcha_flag" value="1" ${g_recaptcha_flag_checked} /> 利用する場合「サイトキー」「シークレットキー」の入力が必要です
			</td>
		</tr>
		<tr>
			<th>サイトキー (Site Key)</th>
			<td>
				<input type="text" name="g_recaptcha_site_key" id="g_recaptcha_site_key"value="${g_recaptcha_site_key}" size="40" /><br />
			</td>
		</tr>
		<tr>
			<th>シークレットキー (Secret Key)</th>
			<td>
				<input type="text" name="g_recaptcha_secret_key" id="g_recaptcha_secret_key" value="${g_recaptcha_secret_key}" size="40" /><br />
			</td>
		</tr>
	</table>
