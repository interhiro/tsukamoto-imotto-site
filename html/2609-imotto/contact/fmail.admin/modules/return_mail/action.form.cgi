...
$body = $gethash{'body'};
$body =~ s/<br \/>/\n/g;
$resend_mail_address = $gethash{'resend_mail_address'};
...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" onSubmit="return checkUserEdit(this,'${submit}');">
	...
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			...
			<th>返信用アドレス</th>
			<td>
				<input type="text" name="resend_mail_address" id="resend_mail_address" value="${resend_mail_address}" onblur="email_check(this.id)" />
				<span>例）info\@freesale.co.jp</span>
				<span class="caution">返信先のメールアドレスとして利用します。</span>
			</td>
		<tr>
