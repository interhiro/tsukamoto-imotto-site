...
if($form{'id'} ne $null){
	...
	*gethash = &setHash(@users_info);
	...
	# 2023.3.9 ver5.043 管理者IDを変更操作可とする対応
	if($form{'power'} eq 'Admin' ) {
		$readonly = "";
	} else {
		$readonly = " readonly";
	}
...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" enctype="multipart/form-data" onSubmit="return checkUserEdit(this,'${submit}');">
	...
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			<th>パスワード</th>
			<td><input type="text" name="form_user_password" id="form_user_password" value="" /> <input type="button" value="自動生成" onClick="random_password();" /><span>※8文字以上の英数で入力してください</span></td>
		</tr>
		<tr>
			<th>ユーザ権限</th>
