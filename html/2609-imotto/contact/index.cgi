...
# Google reCaptcha
$g_recaptcha_html = '';
if ($g_recaptcha_flag && $g_recaptcha_site_key && $g_recaptcha_secret_key && !$flag_futurephone) {
	$g_recaptcha_html = '<script src="https://www.google.com/recaptcha/api.js" async defer></script>';
	$g_recaptcha_html .= "<div style=\"margin-bottom:10px;display:flex;justify-content:center;\" class=\"g-recaptcha\" data-sitekey=\"$g_recaptcha_site_key\"></div>";
}
...
if($_GET{'mode'} eq "send" && -f $send_token){
	if (!$redirect) {
		...
		for($cnt_rmail=0;$cnt_rmail<@return_mail_set;$cnt_rmail++){
			($return_mail_id,$return_mail_type,$return_mail_name,$return_mail_element,$return_mail_value,$return_mail_flag,$return_mail_from,$return_mail_sender,$return_mail_subject,$return_mail_serials,$return_mail_email_field,$return_mail_body,$resend_mail_address) = split(/\t/,$return_mail_set[$cnt_rmail]);
			...
			if($return_mail_type){
				...
				if(($return_mail_flag) && $return_mail_email_address ne $null){
					...
					&sendmail($return_mail_email_address,$null,$null,$return_mail_from,$return_mail_sender,$return_mail_subject,$return_mail_body,$resend_mail_address);
				}
			}
			else {
				# 条件で送信
		...
		for($cnt=0;$cnt<@send_conditional;$cnt++){
			...
			if($send_conditional_type){
				...
				if($mail_method eq "html"){
					...
					for($cnt=0;$cnt<@elements;$cnt++){
						...
						if($join_values{$elements_id}){
							...
							$mailform_resend_mail_address =~ s/&lt;${elements_id}&gt;/$join_values{$elements_id}/g;
							...
							$mailform_resend_mail_address =~ s/&lt;${elements_id}&gt;/$getElementById{$elements_id}/g;
				...
				#汚染チェック 返信先
				$mailform_resend_mail_address =~ s/\;/；/g;
				$mailform_resend_mail_address =~ s/\n//g;
				$mailform_resend_mail_address =~ s/\r//g;
	...
	#			Jcode::convert(\$send_subject,"sjis","utf8");
				...
				if(@file_paths > 0 && !($attached_mode)){
					...
					sendAttachMail($mailform_sender_address_from,$send_mailaddress,$cc,$bcc,$mailform_resend_mail_address, $send_subject, $send_mail_body, @file_datas, @file_paths);
				}
				else {
					&sendmail($send_mailaddress,$cc,$bcc,$mailform_sender_address,$mailform_sender_address_from,$send_subject,$send_mail_body, $mailform_resend_mail_address);
				}
			}
			else {
				# 条件で送信
...
#		($conditional_id,$must_name,$error_message,$must_elements,$note) = split(/\t/,$must[$cnt]);
	...
	if ($g_recaptcha_flag && $g_recaptcha_secret_key && !$flag_futurephone) {
		use LWP::UserAgent; # 必要に応じて use 欄に追加してください
		my $ua = LWP::UserAgent->new;
		my $response = $ua->post('https://www.google.com/recaptcha/api/siteverify', {
			secret   => $g_recaptcha_secret_key,
			response => $q->param('g-recaptcha-response'),
			remoteip => $ENV{'REMOTE_ADDR'},
		});

		if ($response->is_success) {
			my $res_content = $response->decoded_content;
			if ($res_content !~ /"success":\s*true/) {
				$send_flag = 0;
				push @session_data, "spam_error=reCAPTCHA認証に失敗しました。チェックを入れてください。";
			}
		} else {
			$send_flag = 0;
			my $debug_info = join('|', 
				...
				$q->param('g-recaptcha-response'), 
				$ENV{'REMOTE_ADDR'}
			);
			push @session_data, "spam_error=${debug_info}";

			# push @session_data, "spam_error=認証サーバーに接続できませんでした。";
		}
	}
