	...
	if($sess=$_SESSION['import_csv'][$_POST['field']]){
		...
		foreach($sess as $s){
			...
			foreach($field as $fid=>$f){
				...
				if ($f['type'] == 'pdf') {
					if ($contribute_data['data'][$_POST['field']][$fid] != $val) {
						$tmp_path = "{$id}_{$fid}"; // PDF保存用のフォルダ名
						$save_dir = DATA_DIR . "/contribute/pdfs/{$tmp_path}/";
						...
						if (!strlen($val)) {
							...
							// PDFアップロード
							$org_filename = $val;
							$basename = basename($matches[1]); // オリジナルファイル名
							$tmp_path = "{$id}_{$fid}"; // PDF保存用のフォルダ名

							@mkdir($save_dir, 0777, true);
							chmod_if_not_suexec($save_dir);

							$file_content = @file_get_contents(ROOT_DIR . $org_filename);
							if ($file_content === false) {
								$val = 'get_file_failed';

							} else {
								data_write("/contribute/pdfs/{$tmp_path}/{$basename}", $file_content);
								$val = $basename;
							}
