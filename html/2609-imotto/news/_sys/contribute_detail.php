...
function delete_dir($dir) {
	...
	foreach ($items as $item) {
		...
		$path = $dir . '/' . $item;
		if (is_dir($path)) {
			delete_dir($path); // 再帰的に
		} else {
			unlink($path); // ファイルなら削除
		}
	}
	rmdir($dir); // 最後に自分を削除
}
...
if(@$_POST['mode']=='edit'){
	if(check_duplicate_dir_contribute($_GET['id'],$_POST['url'])){
		...
		if(isset($_POST['delete_pdf']))foreach ($_POST['delete_pdf'] as $field_index_id => $field) {
			foreach ($field as $field_id => $field_data) {
				if (!empty($field_data)) {
					$folder = DATA_DIR . '/contribute/pdfs/' . $field_data;
					delete_dir($folder); // ディレクトリごと削除
					unset($data['data'][$field_index_id][$field_id]);
				}
			}
		}
		if (@$_FILES['pdf']['name']) {
			foreach ($_FILES['pdf']['name'] as $field_index_id => $field) {
				foreach ($field as $field_id => $field_data) {
					$save_dir = "{$dir}/pdfs/";
					@mkdir($save_dir);
					chmod_if_not_suexec($save_dir);
					if ($_FILES['pdf']['tmp_name'][$field_index_id][$field_id]){
						// $tmp_path = "{$data_id}_{$field_index_id}_{$field_id}/";
						$tmp_path = "{$data_id}_{$field_id}/";
						$save_dir .= $tmp_path;
						@mkdir($save_dir);
						chmod_if_not_suexec($save_dir);

						// オリジナルのファイル名を取得
						$original_name = $_FILES['pdf']['name'][$field_index_id][$field_id];
						$field_save_path = $original_name;

						// ファイル内容を読み込み
						$file_data = @file_get_contents($_FILES['pdf']['tmp_name'][$field_index_id][$field_id]);
						// ファイルパスを構築
						$save_path = $save_dir . $field_save_path;

						// データに保存先ファイル名を記録
						$data['data'][$field_index_id][$field_id] = $field_save_path;
						// ファイル保存
						data_write('contribute/pdfs/' . $tmp_path . $field_save_path ,$file_data);
						// 一時ファイル削除
						unlink($_FILES['pdf']['tmp_name'][$field_index_id][$field_id]);
					}
				}
			}
		}
			...
			<div class="contribute_detail_left">
				<div class="contribute_detail_header">
					...
					<p>
						<label>
							<span>keyword</span>
							<input type="text" name="keyword" value="<?php echo htmlspecialchars(@$contribute['keyword']); ?>" class="keyword" />
						</label>
					</p>
					<p>
						<label>
							<span>description</span>
							<input type="text" name="description" value="<?php echo htmlspecialchars(@$contribute['description']); ?>" class="description" />
						</label>
					</p>
					<p>
						<label>
							<span>h1</span>
							<input type="text" name="h1" value="<?php echo htmlspecialchars(@$contribute['h1']); ?>" class="h1" />
						</label>
					</p>
							...
							<td>
								...
								<?php
									...
									case 'image':
										...
										<?php endif ?>
										<input type="file" name="image[<?php echo $id; ?>][<?php echo $field_id; ?>]" data-maxfilesize="<?php echo ini_get('upload_max_filesize'); ?>" /><br />
								...
								<?php
									...
									case 'pdf':
										if(@$contribute['data'][$id][$field_id]) :
											$pdf_file = "{$data_id}_{$id}_{$field_id}/" . $contribute['data'][$id][$field_id];
										?>
											<div class="pdf_action">
												<a href="../_data/contribute/pdfs/<?php echo $pdf_file;?>" target="_blank"><?php echo $contribute['data'][$id][$field_id];?></a><br />
												<input type="hidden" name="data[<?php echo $id; ?>][<?php echo $field_id; ?>]" value="<?php echo @$contribute['data'][$id][$field_id];?>" />
												<input type="checkbox" name="delete_pdf[<?php echo $id; ?>][<?php echo $field_id; ?>]" value="<? echo "{$data_id}_{$id}_{$field_id}" ?>" />&nbsp;登録済みPDFを削除<br />
											</div>
										<?php endif ?>
										<input type="file" accept="application/pdf" name="pdf[<?php echo $id; ?>][<?php echo $field_id; ?>]" data-maxfilesize="<?php echo ini_get('upload_max_filesize'); ?>" /><br />
										<?php if(ini_get ('upload_max_filesize')) : ?>
										アップロードサイズ上限：<?php echo ini_get ('upload_max_filesize') ?>
									<?php endif ?>
								<?php
...
<script>
	document.querySelectorAll('input[type="file"]').forEach(function(input) {
		input.addEventListener('change', function(e) {
			...
			const maxSizeAttr = e.target.dataset.maxfilesize;

			if (!file || !maxSizeAttr) return;

			// 単位（MBなど）を考慮してパース
			const unit = maxSizeAttr.match(/[a-zA-Z]+/)?.[0]?.toUpperCase() || 'M';
			const number = parseFloat(maxSizeAttr);

			// 単位をバイトに変換
			let maxSizeBytes = number * 1024 * 1024; // デフォルトは MB
			if (unit === 'K') maxSizeBytes = number * 1024;
			if (unit === 'G') maxSizeBytes = number * 1024 * 1024 * 1024;

			if (file.size > maxSizeBytes) {
				alert(`ファイルサイズは ${maxSizeAttr}B 以下にしてください`);
				e.target.value = '';
			}
		});
	});
